...
| 09E | **Forgot-password requests + secretary provisioning + real smoke + local AI test** | Migration additive `password_reset_requests` (RLS chỉ Admin; RPC `request_password_reset` SECURITY DEFINER: trung lập, chống spam 24h, best-effort khớp hồ sơ); trang công khai `/forgot-password` + link ở 2 cổng login; Admin `/admin/password-requests` (cấp mật khẩu tạm/từ chối, audit `RESOLVE/REJECT_PASSWORD_RESET_REQUEST`) + alert PENDING ở dashboard + nav; UUID validate sớm route ảnh; 2 tài khoản Bí thư `0944577905`/`0368103532` (script `provision-secretaries.mjs`, đọc env, must_change_password, chưa phân công + highlight); Gemini dry-run 3 ảnh `src/images` (report gitignored, PII); health phase `09e-…` + 3 cờ | `PROMPT-09E-...report.md` |

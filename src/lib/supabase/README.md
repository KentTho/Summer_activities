...
| --- | --- | --- |
| `client.ts` | Client Components | `createBrowserClient` với publishable key public |
...
2. Copy `.env.example` → `.env.local`, điền `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`
   (tên cũ `NEXT_PUBLIC_SUPABASE_ANON_KEY` vẫn được đọc để backward-compat), `SUPABASE_SERVICE_ROLE_KEY`.
